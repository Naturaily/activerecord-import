...
module ActiveRecord::Import #:nodoc:
  class Result < Struct.new(:failed_instances, :num_inserts, :row_ids)
  end
  ...
  class << self
    ...
    def import( *args )
      ...
      return_obj = if is_validating
        ...
        num_inserts, row_ids = import_without_validations_or_callbacks( column_names, array_of_attributes, options )
        ActiveRecord::Import::Result.new([], num_inserts, row_ids)
      end
    ...
    def import_with_validations( column_names, array_of_attributes, options={} )
      ...
      num_inserts, row_ids  = if array_of_attributes.empty? || options[:all_or_none] && failed_instances.any?
                      [0, []]
                    else
                      import_without_validations_or_callbacks( column_names, array_of_attributes, options )
                    end
      ActiveRecord::Import::Result.new(failed_instances, num_inserts, row_ids)
    end
    ...
    def import_without_validations_or_callbacks( column_names, array_of_attributes, options={} )
      ...
      if not supports_import?
        ...
        # perform the inserts
        number_inserted, row_ids = connection.insert_many( [ insert_sql, post_sql_statements ].flatten,
                                                  values_sql,
                                                  "#{self.class.name} Create Many Without Validations Or Callbacks" )
      end
      [number_inserted, row_ids]
    end
