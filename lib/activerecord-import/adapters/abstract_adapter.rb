module ActiveRecord::Import::AbstractAdapter
  ...
  module InstanceMethods
    ...
    def insert_many( sql, values, *args ) # :nodoc:
      ...
      sql_result = if NO_MAX_PACKET == max or total_bytes < max
        number_of_inserts += 1
        sql2insert = base_sql + values.join( ',' ) + post_sql + " RETURNING ID"
      ...
      row_ids = sql_result.map(&:values).flatten

      [number_of_inserts, row_ids]
    end
